...
parser.add_argument('--data_config_file',type=str, default=None) #'log/Backbone/RL_hard_share/3R/schedule_MTL.ini')
...
parser.add_argument("--pad_to",type=int,default=None) # 0
...
def test(args):
    ...
    print("model task", model.all_tasks)
    print("data task:", POPEN.cycle_set)
    ...
    loader_set = {}
    if args.data_config_file is None:
        args.data_config_file = args.config_file  
    task_dict = {
        'RP_293T': 'HEK',
        'RP_muscle': 'Muscle',
        'RP_PC3': 'pc3'
    ...
    for task in POPEN.cycle_set:
        ...
        if (task in ['RP_293T', 'RP_muscle', 'RP_PC3']):
            if base_csv.endswith("sequence.csv"):
                task_name = task_dict[task]
            else:
                task_name = task
            # base_csv = 'cycle_protein_coding.csv'
            datapopen = Auto_popen(args.data_config_file)
            datapopen.csv_path = base_csv.replace("cycle",task_name)
            datapopen.kfold_index = args.kfold_index
