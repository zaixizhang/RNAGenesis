...
from models.diffusion_models.util import register_model
from .fp16_util import convert_linear_to_f16, convert_linear_to_f32
...
@register_model('DiT')
class SeqDiT(ModelMixin, ConfigMixin):
    """
    ...
    def __init__(
        ...
        self.dtype_ = th.float16 if use_fp16 else th.float32
        hidden_size = in_channels if hidden_size is None else hidden_size
        self.preprocess = False
        self.add_residual = add_residual
        if hidden_size != in_channels:
  ...
  # use linear layer
    ...
    def forward(self, x, t, y=None, use_cross_attention = False, **unused):
        """
        Forward pass of DiT.
        x: (N, T, D) tensor of sequential inputs 
        ...
        if self.preprocess:
            residual = x
            x = self.norm(x)  
            x = self.proj_in(x) # (N, T, D_in) -> (N, T, D)

        # x = x.squeeze(1)  #  (N, T, D) # [0623] Removed by YYK
        ...
        # x_type = x.dtype
        x, c = x.to(self.dtype_), c.to(self.dtype_)
        ...
        # x = x.unsqueeze(1)   # (N, 1, T, D)  # [0623] Removed by YYK
        # return x
        if self.preprocess:
            x = self.proj_out(x) # (N, T, D_in)
