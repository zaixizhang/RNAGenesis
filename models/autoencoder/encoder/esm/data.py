...
class BatchConverter(object):
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]): 
        ...
        # [YYK]: Support the data without masking
        item_num = len(raw_batch[0])
        if item_num == 4:
    ...
    #             print(tokens, masked_tokens)
            ...
            labels = []
            strs = []
            seq_encoded_list =["None"] * batch_size 
            for i, (label, seq_str, seq_encoded) in enumerate(
                ...
                # seq = torch.tensor(seq_encoded, dtype=torch.int64)
                seq = [self.alphabet.get_idx(s) for s in seq_str]
                if self.alphabet.unk_idx in seq:   
                    print(set(seq_str))
                seq = torch.tensor(
                ...
                tokens[
                    i,
                    int(self.alphabet.prepend_bos) : len(seq) #len(seq_encoded)
                    + int(self.alphabet.prepend_bos),
                ] = seq
            
                if self.alphabet.append_eos:
                    # tokens[i, len(seq_encoded) + int(self.alphabet.prepend_bos)] = self.alphabet.eos_idx
                    tokens[i, len(seq) + int(self.alphabet.prepend_bos)] = self.alphabet.eos_idx
